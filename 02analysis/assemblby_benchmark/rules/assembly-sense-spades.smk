rule assembly_precision_sense_spades:
    input:
        contigs=f'{config["rdir"]}/assembly/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.assm.spades.{{config}}.fasta',
    output:
        tsv=f'{config["rdir"]}/assembly-sense/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mmseqs.spades.{{config}}/dummy.txt',
    wildcard_constraints:
        assm_input_1="\w+",
        assm_input_2="\w+",
        config="\w+",
    params:
        tmp=f'{config["rdir"]}/assembly-sense/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mmseqs.spades.{{config}}_tmp',
        result=f'{config["rdir"]}/assembly-sense/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mmseqs.spades.{{config}}',
    ...
    log:
        mmseqs_log=f'{config["rdir"]}/assembly-sense/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mmseqs.spades.{{config}}.log',
    benchmark:
        f'{config["rdir"]}/benchmarks/assembly-sense/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mmseqs.spades.{{config}}.bmk'
    message:
