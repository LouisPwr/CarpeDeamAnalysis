rule assembly_mapping_evaluation_megahit:
    input:
    ...
    output:
        ...
        flagstat=f'{config["rdir"]}/assembly-mapping/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mapping.megahit.{{config}}.stats.txt',
    wildcard_constraints:
        assm_input_1="\w+",
        assm_input_2="\w+",
        config="\w+",
    params:
        sam=f'{config["rdir"]}/assembly-mapping/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mapping.megahit.{{config}}.sam',
        bam=f'{config["rdir"]}/assembly-mapping/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mapping.megahit.{{config}}.bam',
        ...
        bt_index=f'{config["rdir"]}/assembly-mapping/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mapping.megahit.{{config}}',
    threads: config["bowtie2_threads"]
    log:
        mapping_log=f'{config["rdir"]}/assembly-mapping/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mapping.megahit.{{config}}.log',
    benchmark:
        f'{config["rdir"]}/benchmarks/assembly-mapping/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mapping.megahit.{{config}}.bmk'
    conda:
        "/miniconda3/envs/bowtie2"
    message:
        """--- bowtie2 mappping of megahit assemblies. """
    shell:
        """
