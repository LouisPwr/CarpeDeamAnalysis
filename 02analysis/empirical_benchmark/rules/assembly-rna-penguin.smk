rule assembly_rna_evaluation_penguin:
    input:
    ...
    output:
        tsv=f'{config["rdir"]}/assembly-rna/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.rna.penguin.{{config}}.tsv',
    wildcard_constraints:
        assm_input_1="\w+",
        assm_input_2="\w+",
        config="\w+",
    params:
        tmp=f'{config["rdir"]}/assembly-rna/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.rna.penguin.{{config}}_tmp',
        mmseqs_bin=config["mmseqs_bin"],
        features=f'{config["rdir"]}/assembly-annotation-eval/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.proteins.penguin.{{config}}/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.proteins.penguin.{{config}}.ffn',
        rna=f'{config["rdir"]}/assembly-rna/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.rna.penguin.{{config}}.fa',
        rna_clust=f'{config["rdir"]}/assembly-rna/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.rna.penguin.{{config}}.clust',
        wdir=config["wdir"],
    ...
    log:
        mmseqs_log=f'{config["rdir"]}/assembly-rna/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.rna.penguin.{{config}}.log',
    benchmark:
        f'{config["rdir"]}/benchmarks/assembly-rna/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.rna.penguin.{{config}}.bmk'
    message:
