...
rule assembly_spades:
    input:
        ...
        reads1=(
        lambda wildcards: f'{config["sanadir"]}/{sample_table_read.fw_reads[wildcards.smp]}'
        ),
        reads2=(
        lambda wildcards: f'{config["sanadir"]}/{sample_table_read.rev_reads[wildcards.smp]}'
        )
    output:
    ...
    shell:
        ...
        #N=$(grep -c '>' {params.tmp_dir}/contigs.fasta)
