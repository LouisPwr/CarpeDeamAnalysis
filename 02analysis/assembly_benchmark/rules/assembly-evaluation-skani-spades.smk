rule assembly_skani_evaluation_spades:
    input:
        contigs=f'{config["rdir"]}/assembly/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.assm.spades.{{config}}.fasta',
    output:
        skani=f'{config["rdir"]}/assembly-skani/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.genes.spades.{{config}}.ani.tsv',
        searchskani=f'{config["rdir"]}/assembly-skani/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.genes.spades.{{config}}.search-skani.tsv',
    ...
    log:
        skani_log=f'{config["rdir"]}/logs/assembly-skani/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.skani.spades.{{config}}.log',
    benchmark:
        f'{config["rdir"]}/benchmarks/assembly-skani/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.skani.spades.{{config}}.bmk'
    message:
        """--- skani """
    shell:
        """
