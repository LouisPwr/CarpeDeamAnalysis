rule assembly_mmseqs_evaluation_spades:
    input:
    ...
    output:
        tsv=f'{config["rdir"]}/assembly-mmseqs/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mmseqs.spades.{{config}}.tsv',
    wildcard_constraints:
        assm_input_1="\w+",
        assm_input_2="\w+",
        config="\w+",
    params:
        tmp=f'{config["rdir"]}/assembly-mmseqs/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mmseqs.spades.{{config}}_tmp',
    ...
    log:
        mmseqs_log=f'{config["rdir"]}/assembly-mmseqs/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mmseqs.spades.{{config}}.log',
    benchmark:
        f'{config["rdir"]}/benchmarks/assembly-mmseqs/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.mmseqs.spades.{{config}}.bmk'
    message:
