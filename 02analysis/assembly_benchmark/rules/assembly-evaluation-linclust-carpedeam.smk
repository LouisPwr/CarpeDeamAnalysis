rule assembly_linclust_evaluation_carpedeam:
    input:
    ...
    output:
        cluster=f'{config["rdir"]}/assembly-linclust/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.carpedeam.{{config}}_rep_seq.fasta',
    wildcard_constraints:
        assm_input_1="\w+",
        assm_input_2="\w+",
        config="\w+",
    params:
        tmp=f'{config["rdir"]}/assembly-linclust/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.carpedeam.{{config}}_tmp',
        ...
        # )
        clusterID=f'{config["rdir"]}/assembly-linclust/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.carpedeam.{{config}}',
    threads: config["mmseqs_threads"]
    log:
        mmseqs_log=f'{config["rdir"]}/assembly-linclust/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.carpedeam.{{config}}.log',
    benchmark:
        f'{config["rdir"]}/benchmarks/assembly-linclust/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.carpedeam.{{config}}.bmk'
    message:
