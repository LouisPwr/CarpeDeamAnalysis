rule assembly_skani_evaluation_carpedeam:
    input:
        contigs=f'{config["rdir"]}/assembly/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.assm.carpedeam.{{config}}.fasta',
    output:
        skani=f'{config["rdir"]}/assembly-skani/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.genes.carpedeam.{{config}}.ani.tsv',
        searchskani=f'{config["rdir"]}/assembly-skani/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.genes.carpedeam.{{config}}.search-skani.tsv',
    ...
    log:
        skani_log=f'{config["rdir"]}/logs/assembly-skani/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.skani.carpedeam.{{config}}.log',
    benchmark:
        f'{config["rdir"]}/benchmarks/assembly-skani/{{smp}}.{{assm_input_1}}-{{assm_input_2}}.skani.carpedeam.{{config}}.bmk'
    message:
        """--- skani """
    shell:
        """
